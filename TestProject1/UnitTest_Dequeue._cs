...
namespace TestProject
{
    ...
    public class AlgorithmsDataStructures
    {
        ...
        //[ExpectedException(typeof(ArgumentOutOfRangeException))]
        [TestCategory("Тесты")]
        public void Dequeue_IsPalindrom()
        {
            try
            {
                Deque<string> dequeue = new Deque<string>();
                
                Assert.AreEqual(dequeue.IsPalindrome("рор"), true);
                Assert.AreEqual(dequeue.IsPalindrome(" "), true);
                Assert.AreEqual(dequeue.IsPalindrome(""), true);
                Assert.AreEqual(dequeue.IsPalindrome("palindrom"), false);
            }
            catch (IndexOutOfRangeException e)
            {
                Assert.Fail();
